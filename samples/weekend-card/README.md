...
기존 Workshop에 더하는 **선택 샘플**입니다. 다섯 과제·스타터·진행 순서를 바꾸지 않습니다.
...
### 서비스 흐름 및 Azure 확장 아키텍처

**서비스 흐름:** 공개 데이터 수집 → 날짜·취향별 코스 제안 → 장소 고정·재추천 → 알림 초안 확인 → 승인 후 채널 전송 → 공식 링크에서 사용자 예약.

[![주말 한 장 Azure 확장 아키텍처 — Functions, Cosmos DB, App Service, Foundry, Logic Apps](architecture.png)](architecture.png)

[아키텍처 원본 SVG](architecture.svg) · [Azure 공식 아이콘 출처·사용 조건](../../docs/sources.md)
...
동봉 스냅샷 기준의 고정 입력으로 진행합니다. 아래 날짜는 실습 재현용이며 실제 외출 예약·발송 날짜로 사용하지 않습니다. API를 새로 수집했다면 장소와 기간이 달라질 수 있으므로 현재 데이터를 먼저 확인합니다.

| 단계 | 입력·조작 | 기대 결과 |
|---|---|---|
| 코스 조회 | 날짜 `2026-09-19`, 전체 지역, 반나절, `초록 산책`, 두 필터 해제 | 스냅샷의 공원·문화공간 후보 표시; 해당 날짜 밖의 행사 제외 |
| 선호 유지 | 공원 한 곳의 `여기는 꼭` 선택 후 `다른 조합 보기` | 고정 장소가 새 조합에도 유지 |
| 조건 충돌 | 공원 고정 상태에서 `문화공간만 보기` 선택 | 충돌 사유와 고정 해제 안내; 공원을 임의로 실내 시설로 취급하지 않음 |
| 결과 저장 | 고정 해제 후 코스 선택 → `알림용 코스 파일 받기` | 요청 파일만 다운로드; 메시지 발송 없음 |
| 초안 검토 | 아래 명령으로 별도 초안 생성 | 날짜·장소·공식 링크·제한 사항 출력; 채널 발송 없음 |

내려받은 `weekend-request.json`을 샘플 폴더에 둔 뒤 실행합니다. 기존 출력이 있으면 덮어쓰지 말고 새 출력 이름을 지정합니다.

```powershell
python agent.py draft --request .\weekend-request.json --mode rules --output .\data\workshop-preview.json
```

**실행·판단·승인 구분:** 공개 데이터 적재와 날짜·거리 필터는 코드가 수행하고, 모델 연결 경로에서는 허용된 후보 중 선택·설명을 AI가 수행합니다. 알림은 수신처·전문 확인 후 사용자가 승인하며 실제 예약은 공식 링크에서 처리합니다. 이 시연은 모델·채널 연결 없이 진행할 수 있고, 규칙 기반 결과를 실제 AI 실행으로 표현하지 않습니다.
...
**시연 범위 판정:** `rules` 결과는 로컬 규칙 실행, 모의 테스트는 코드 경로 확인, `azure`의 실제 응답은 모델 연결 확인으로 구분합니다. 실제 연결을 실행하지 않은 경우 결과를 보류하고 계정·권한·설정 등 차단 원인을 남깁니다.

## 6. Teams·Discord 알림 전송
