...
**강사와 함께 이 페이지를 위에서 아래로 읽고, 선택한 아이디어를 직접 만들어봅니다.**

## 1. 오늘 할 일

강사가 제시하는 **업무 아이디어 5개 중 하나를 선택**하고, 자연어로 제작을 요청합니다. 직접 실행하고 동료의 의견을 듣고 수정합니다. 웹앱·업무 도구·에이전트 등 결과물 형태와 추가 기능은 자유롭게 정합니다.
...
사용량이 제한된 무료·체험 플랜만으로 전체 실습이 가능하다고 보장하지 않습니다. 실제로 첫 파일 생성·수정까지 해보고 기능·잔여 사용량을 확인합니다. 제품별 조건은 [공식 문서](docs/04-models-and-tools.md)를 참고합니다.
...
## 5. 실습 시작 — 새 작업 폴더와 코딩 도구

1. 파일 탐색기에서 새 폴더를 만듭니다. 예: `MyVibeProject`. 기존 업무 자료 폴더는 사용하지 않습니다.
2. VS Code를 사용하는 경우 **File → Open Folder**에서 이 폴더를 엽니다.
3. 강사가 지정한 경로 하나로 코딩 도구를 시작합니다.

| 지정 도구 | 시작 방법 |
|---|---|
| GitHub Copilot — VS Code | Chat을 열고 **Agent 모드** 선택, 지정 GitHub 계정으로 로그인 |
| GitHub Copilot CLI | 새 폴더에서 터미널을 열고 `copilot` 실행, 안내에 따라 로그인 |
| Codex CLI | 새 폴더에서 터미널을 열고 `codex` 실행, 승인된 방식으로 로그인 |

설치되지 않았으면 [도구별 설치·로그인 안내](docs/04-models-and-tools.md)의 지정 경로만 따릅니다. 오류가 나면 관리자 정책을 해제하지 말고 강사에게 알려주세요.

이 저장소는 읽는 교재입니다. **공통 실습을 시작하려고 예제 Python 코드를 복제하거나 함수 빈칸을 채울 필요는 없습니다.**

**코딩 도우미에 입력**

```text
현재 작업 폴더를 확인해줘.
나는 비개발자이며 이 폴더에서 새 프로젝트를 만들 거야.
아직 파일 수정·설치·클라우드 리소스 생성은 하지 마.
어떤 파일에 접근할 수 있는지 간단히 설명해줘.
```

**확인:** 도구가 응답하고 의도한 새 폴더를 인식합니다.
...
먼저 나와 요구사항을 정리해줘. 모호한 부분은 질문을 하나씩 해줘.
첫 버전은 가장 작은 기능 하나에 집중하고 작은 합성 자료를 사용해.
웹앱·업무 도구·에이전트 중 어떤 형태가 맞는지 이유를 설명해줘.
DB나 유료 AI 모델을 처음부터 필수로 가정하지 마.
필요한 프로그램·서비스와 대안을 설명한 후 구현 계획을 보여줘.
내가 확인하기 전에는 설치·외부 전송·유료 리소스 생성을 하지 마.
```
...
```text
이 계획으로 첫 기능을 만들어줘.
어떤 파일을 만들고 바꿀지 알려주고, 현재 폴더 안에서 작업해.
끝나면 내가 직접 실행할 명령 또는 열어야 할 파일을 순서대로 알려줘.
실제 동작과 아직 모의로 만든 부분을 구분해줘.
```

**확인:** 누구의 문제를 해결하는지, 첫 기능이 무엇인지 분명하고 실제 파일이 생성됩니다. 화면·언어·기능이 옆 사람과 달라도 괜찮습니다.
...
Azure 모델·SQL 조합을 선택했을 때만 [상세 구성 예제](docs/01-environment.md)를 참고합니다. 실제 구독·테넌트 접근과 비용은 사전 준비에서 정한 범위를 따릅니다.
...
[설치·로그인](docs/04-models-and-tools.md) · [문제 해결](docs/troubleshooting.md) · [공식 사업 자료](docs/sources.md) · [운영자 안내](docs/facilitator.md)

기존 Azure·SQL 기반 Python 코드는 **선택 참고 예제**입니다. 자유 제작의 필수 과제가 아닙니다: [예제 실행 안내](docs/02-first-run.md).

**자료 상태:** 공통 설명·따라하기와 선택 예제 코드를 제공합니다. 실제 코딩 도구 화면·Azure 통합 실행 스크린샷은 아직 미촬영이며, 실제 Azure 통합 실행도 리허설 전입니다.
